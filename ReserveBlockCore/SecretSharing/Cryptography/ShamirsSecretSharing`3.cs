namespace ReserveBlockCore.SecretSharing.Cryptography
{
        ...
        where TExtendedGcdResult : struct, IExtendedGcdResult<TNumber>
    {
        ...
        public Shares<TNumber> MakeShares(TNumber numberOfMinimumShares, TNumber numberOfShares, int securityLevel)
        {
            ...
            if (min < 2)
            {
                throw new ArgumentOutOfRangeException(nameof(numberOfMinimumShares), numberOfMinimumShares, "");
            }

            if (min > max)
            {
                throw new ArgumentOutOfRangeException(nameof(numberOfShares), numberOfShares, "");
            }
